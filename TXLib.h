...
//=================================================================================================================
//           [These sections are for folding control  in Code::Blocks]         [$Date: 2024-03-24 01:30:00 +0400 $]
...
#define _TX_VER      _TX_v_FROM_CVS ($VersionInfo: , TXLib.h, 00173a, 173, 2024-03-24 01:30:00 +0400, "Ded (Ilya Dedinsky, http://txlib.ru) <mail@txlib.ru>", $)
#define _TX_VERSION  _TX_V_FROM_CVS ($VersionInfo: , TXLib.h, 00173a, 173, 2024-03-24 01:30:00 +0400, "Ded (Ilya Dedinsky, http://txlib.ru) <mail@txlib.ru>", $)
#define _TX_AUTHOR   _TX_A_FROM_CVS ($VersionInfo: , TXLib.h, 00173a, 173, 2024-03-24 01:30:00 +0400, "Ded (Ilya Dedinsky, http://txlib.ru) <mail@txlib.ru>", $)
...
//!                        оригинальный размер рисунка из файла.</i>
//! @param   imageType  <i>Тип загружаемого изображения, см. ниже. Необязателен.  Если не указан, загружается
...
//!
//! @note
//!       -# Изображения поддерживаются <b>только в формате BMP.</b> Если взять файл другого формата, например JPG,
...
//!          @nn
//!       -# Если файл по-прежнему не загружается, <b>откройте его в редакторе Microsoft Paint</b> (нажмите
//!          <tt>\<WIN+R\> mspaint \<ENTER\></tt>) или другой программе обработки изображений и <b>сохраните его
//!          заново как изображение в формате BMP.</b> Это иногда бывает нужно для файлов, преобразованных из других
...
//!          @nn
//!       -# <b>Не надо часто загружать</b> одно и то же изображение, особенно в цикле. От этого программа начинает
//!          тормозить! @n
...
long WINAPI _txVectoredExceptionHandler (EXCEPTION_POINTERS* exc)
    ...
    if (!_txProcessSystemWarnings)
        ...
        return (!(exc && exc->ExceptionRecord && exc->ExceptionRecord->ExceptionFlags & EXCEPTION_NONCONTINUABLE))?
...
long _txOnExceptionSEH (EXCEPTION_POINTERS* exc, const char func[])
    ...
    if (code == DBG_PRINTEXCEPTION_C                 ||
        ...
        #ifdef  _TX_SKIP_EXCEPTION
        code == _TX_SKIP_EXCEPTION                   ||
        #endif
...
$   switch (code)
        ...
        GET_DESCR_ (EXCEPTION_CPP_MSC,                  " "  "Исключение MSC С++, вызванное оператором throw.")
        GET_DESCR_ (EXCEPTION_CPP_GCC,                  " "  "Исключение GNU С++, вызванное оператором throw.")
        GET_DESCR_ (EXCEPTION_CPP_GCC_UNWIND,           " "  "Исключение GNU С++, вызванное во время раскрутки стека (rethrow?).")
        GET_DESCR_ (EXCEPTION_CPP_GCC_FORCED,           " "  "Исключение GNU С++, вызванное нарушением магии.")
...
HDC txLoadImage (const char filename[], int sizeX /*= 0*/, int sizeY /*= 0*/,
                 unsigned imageType /*= IMAGE_BITMAP*/, unsigned loadFlags /*= LR_LOADFROMFILE*/)
    {
...
$   HBITMAP image = (HBITMAP) Win32::LoadImage ((loadFlags & LR_LOADFROMFILE)? NULL : GetModuleHandle (NULL),
                                                 filename, imageType, sizeX, sizeY, loadFlags | LR_CREATEDIBSECTION);
